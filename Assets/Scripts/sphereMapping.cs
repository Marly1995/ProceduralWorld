...
public class sphereMapping : MonoBehaviour
{
    ...
    private void GenerateVerts()
    {
        ...
        for (int y = 0; y <= ySize; y++)
        {
            ...
            {
                SetVertex(i++, xSize, y, z);
            }
            for (int x = xSize - 1; x >= 0; x--)
            {
                SetVertex(i++, x, y, zSize);
            }
    ...
    private int CreateBoxTop(int[] triangles, int t, int ring)
    {
        ...
        {
            t = MakeQuad(triangles, t, vMin, vMid, vMin - 1, vMid + xSize - 1);
            ...
            t = MakeQuad(triangles, t, vMid, vMax, vMid + xSize - 1, vMax + 1);
        }
    ...
    private int CreateBoxBot(int[] triangles, int t, int ring)
    {
        ...
        for (int z = 1; z < zSize - 1; z++, vMin--, vMid++, vMax++)
        {
            t = MakeQuad(triangles, t, vMin, vMid + xSize - 1, vMin + 1, vMid);
            for (int x = 1; x < xSize - 1; x++, vMid++)
            {
                t = MakeQuad(
                    triangles, t,
            ...
            t = MakeQuad(triangles, t, vMid + xSize - 1, vMax + 1, vMid, vMax);
        }
