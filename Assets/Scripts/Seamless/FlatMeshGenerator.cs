...
//function to take a noisemap and adjust a flat terrainmesh according to that map.
    public static SeamlessMeshData GenerateTerrainMesh(Noise2D  noisemap, float multiplier, AnimationCurve adjustment)
    {

        float topLeftX = (noisemap.Width - 1) / 2f;
        float topLeftZ = (noisemap.Height - 1) / 2f;
        //noisemap.GeneratePlanar();
        SeamlessMeshData meshData = new SeamlessMeshData(noisemap.Width, noisemap.Height);
        int vertexIndex = 0;
...
public class SeamlessMeshData
{
    ...
    public SeamlessMeshData(int meshWidth, int meshHeight)
    {
